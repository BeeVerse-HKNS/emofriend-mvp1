...
class LogicalBoolean:
    def __init__(self) -> None:
        self._formula = "^(R)"
        self._capability = "logical_boolean"

    def logical_boolean(self, input_data: dict) -> dict:
        try:
            result = self._process(input_data)
            logger.info("logical_boolean_success", capability=self._capability)
            return {"status": "success", "capability": self._capability, "result": result, "formula": self._formula}
        except Exception as exc:
            logger.error("logical_boolean_failed", capability=self._capability, error=str(exc))
